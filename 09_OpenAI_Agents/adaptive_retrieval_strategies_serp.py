...
# =====================================================================
# Same as adaptive_retrieval_strategies.py, but search_web is backed by
# SerpAPI (Google search results) instead of Tavily. One agent, several
# retrieval strategies - which one gets used on any given request is the
# AGENT's own decision (via which tool(s) it picks and how many times),
# not a branch written in this file. This covers 5 of the 6 strategies
# from the slide with real tools (skips a literal SQL-database example -
# the same "pick the right source" idea is already shown by web vs.
# knowledge-base vs. calculator):
...
def search_web(query: str) -> str:
    ...
    return "\n".join(
        f"- {r.get('title', '')} ({r.get('link', '')}): {r.get('snippet', '')[:200]}" for r in organic
    )
