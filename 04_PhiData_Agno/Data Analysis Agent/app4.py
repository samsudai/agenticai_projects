...
if uploaded_file is not None:
    try:
        ...
        if file_type in ["csv", "xlsx", "xls"]:
            ...
            # response = agent.run(f"Find datasets similar to sales data from Q2 in path {temp_path}")
            # response = agent.run(f"What is the minimum recent salary among all employee salaries in {temp_path}?")
            response = agent.run(f"What is the mean unit price among all orders in {temp_path}")
